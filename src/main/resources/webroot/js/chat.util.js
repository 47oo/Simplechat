					...
					window.CHAT = {
						...
						openFace : function() {
							...
							for (var i = 1; i <= 130; i++) {
								...
								faceIcon += '<span class="face-item" onclick="CHAT.selectFace(\''
										+ path + '\')">'
						...
						sendFlower : function() {
							if (CHAT.socket.readyState == WebSocket.OPEN) {
								var msg = "[FLOWER][" + new Date().getTime()
										+ "][" + CHAT.nickname + "]";
								CHAT.socket.send(msg);
							} else {
						...
						init : function(nickname) {
							...
							var appendToPanel = function(msg) {
								...
								if (cmd == "SYSTEM") {
									...
									// 添加一个系统时间标签
									CHAT
											.addSystemTip('<span class="time-label">'
